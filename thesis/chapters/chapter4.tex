...
\begin{enumerate}
	\item Ścieżka Hamiltona w grafie skierowanym,
	\item Ścieżka Hamiltona w grafie nieskierowanym,
	\item Maksymalna klika (ang. \textit{Maximum Clique Size Problem}), 
	\item Maksymalny zbiór niezależny (ang. \textit{Maximum Independent Set}),
	\item Minimalne pokrycie wierzchołkowe (ang. \textit{Minimal Vertex Cover}),
	\item Kolorowanie grafu (ang. \textit{Graph Coloring}),
	\item Problem Komiwojażera (ang. \textit{Traveling Salesperson Problem}),  
	oraz 
	\item Problem sumy podzbioru (ang. Subset Sum).    
\end{enumerate}
...
\lstinputlisting[caption={Funkcja $\main$ w $\maxclique$}]{./codes/main_maxclique.py}
...
\lstinputlisting[caption={Funkcja $\main$ w $\maxindset$}]{./codes/main_maxindset.py}
...
\lstinputlisting[caption={Funkcja $\checkvertexcover$}]{./codes/check_vertexcover.py}
...
\lstinputlisting[caption={Funkcja $\main$ programu $\vertexcover$}]{./codes/main_vertexcover.py}
...
Importuje niezbędne moduły i funkcje, w tym bibliotekę \textit{networkx} do manipulacji grafami oraz \textit{matplotlib.pyplot} do rysowania grafów.
...
\lstinputlisting[caption={Funkcja $\main$ w $\graphcoloring$}]{./codes/main_graphcoloring.py}
