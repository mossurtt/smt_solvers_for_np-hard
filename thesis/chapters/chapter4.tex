...
\lstinputlisting{./codes/read_graph_from_file.py}[caption={Czytanie grafu z pliku}]
\vspace{0.5cm}
...
\lstinputlisting{./codes/read_digraph_from_file.py}[caption={Czytanie skierowanego grafu z pliku}]
\vspace{0.5cm}
...
\lstinputlisting{./codes/read_wgraph_from_file.py}[caption={Czytanie grafu z wagami}]
\vspace{0.5cm}
...
\lstinputlisting{./codes/read_set_from_file.py}[caption={Czytanie zbioru z pliku}]
\vspace{1cm}
...
\lstinputlisting{./codes/proper_numbers.py}
\vspace{0.5cm}

Kolejną funkcją jest $\distinctvs$, która zapewnia, że każdy wierzchołek grafu jest odwiedziany dokładnie raz, czyli żadne dwa wierzchołki nie mają tej samej wartości:

\lstinputlisting{./codes/distinct_vs.py}
\vspace{0.5cm}
...
\lstinputlisting{./codes/dir_edge.py}
\vspace{0.5cm}
...
\lstinputlisting{./codes/wedge.py}
...
$\propernumbers(n)$ gwarantuje, że każda zmienna \(v_j\) znajduje się w zakresie od 0 do $n − 1$, gdzie \(n\) jest rozmiarem zbioru zmiennych.

\begin{align*}
	\propernumbers(n) = \left( \bigwedge_{j=0}^{n-1} (v_j \geq 0 \land v_j < n) \right) 
\end{align*}

$\distinctvs(n)$ zapewnia, że każda para zmiennych \(v_i\) i \(v_j\) jest różna od siebie, eliminując powtarzające się wartości między zmiennymi.

\begin{align*}
	\distinctvs(n) = \left( \bigwedge_{i=0}^{n-1} \bigwedge_{j=i+1}^{n} (v_i \neq v_j) \right)
\end{align*}

$\diredges(n,E)$ sprawdza, czy istnieją krawędzie w grafie o zadanych wierzchołkach \(s\) i \(t\), zapewniając, że dla każdej zmiennej \(v_i\) istnieje skierowana krawędź od niej do kolejnej zmiennej \(v_{i+1}\).​
...
\lstinputlisting{./codes/check_hampath.py}[caption={Funkcja $\checkhampath$}]
...
\lstinputlisting{./codes/check_clique.py}
...
\lstinputlisting{./codes/check_maxindset.py}

W programie \textit{maxindset.py} funkcja $\main$ ma analogiczną rolę do funkcji \textit{\textbf{main}} z programu \textit{maxclique.py}. Obydwie funkcje mają taką samą strukturę kontrolną i wykonują te same czynności, ale w odniesieniu do różnych problemów. 
...
\lstinputlisting{./codes/check_coloring.py}
...
\lstinputlisting{./codes/check_tsp.py}
...
\begin{align*}
	\SubsetSum(n, t, k) = \left( \bigwedge_{j=0}^{n-1} (x_j = 0 \lor x_j = 1) \right) \land 
	\left( \sum_{i=0}^{n-1} (x_i * s_i) = t \right) \land 
